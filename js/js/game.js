...
const P_HEIGHT = 160;
const RUN_CELLH = 294;
const IDLE_CELLH = 380;
const ATTACK_CELLH = 279;
...
function scaleFor(state) {
  if (state === 'idle') return P_HEIGHT / IDLE_CELLH;
  if (state === 'attack') return P_HEIGHT / ATTACK_CELLH;
  return P_HEIGHT / RUN_CELLH;
}
...
function render() {
  ...
  if (paused && mode === 'play') {
    ...
    ctx.fillText('按 P 继续', VW / 2, VH / 2 + 30);
  }
}
